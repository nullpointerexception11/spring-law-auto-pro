...
const formatSize = (size = 0) => `${(size / 1024).toFixed(1)} KB`;

const DocumentCard = memo(function DocumentCard({ doc }) {
  const summary = useMemo(() => {
    return {
      sizeLabel: formatSize(doc.size),
      typeLabel: doc.contentType?.split('/')[1] || 'Bilinmiyor',
      ocrLabel: doc.ocrDocument ? 'OCR TAMAM' : 'OCR BEKLİYOR',
    };
  }, [doc.contentType, doc.ocrDocument, doc.size]);
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-[10px] text-muted-foreground font-medium uppercase mt-1">
            {summary.sizeLabel} &bull; {summary.typeLabel}
          </p>
          ...
          <div className="flex items-center gap-2 mt-3">
            <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-emerald-100 dark:bg-emerald-900/30 text-[10px] font-medium text-emerald-700 dark:text-emerald-300">
              <FileCheck className="h-3 w-3" /> {summary.ocrLabel}
            </span>
            {doc.aiResult && (
...
function DocumentManagerComponent({ matterId }) {
...
export const DocumentManager = memo(DocumentManagerComponent);
